...
1. Copia `.env.example` a `.env` y rellena:
   ...
   - `OPENAI_API_KEY`
   - `JWT_SECRET`
   - `QUEUES_ENABLED` (opcional, `true` para activar BullMQ)
   - `REDIS_URL` (opcional, ejemplo `redis://localhost:6379`)
2. Levanta Postgres con Docker:
   ```bash
   docker compose up -d
   ```
   Para colas, asegura Redis:
   ```bash
   # Usando el docker-compose incluido (agrega servicio redis)
   docker compose up -d redis
   # o instala Redis localmente y usa REDIS_URL=redis://localhost:6379
   ```
...
4. Arranca en desarrollo:
   ...
   npm run dev
   ```
   Si activaste colas, establece en `.env`:
   ```env
   QUEUES_ENABLED=true
   REDIS_URL=redis://localhost:6379
   ```
   Con colas activadas, `/api/upload` responderá `202 Accepted` y encolará el análisis, notificando por Telegram/WhatsApp cuando termine.
