...
def ocr_with_rapidocr(img_path: str) -> Tuple[List[str], str]:
    RapidOCR = try_import_rapidocr()
    if RapidOCR is None:
        return [], "rapidocr_onnxruntime not installed"
    try:
        engine = RapidOCR()
        result, _ = engine(img_path)
        lines = [r[1][0] for r in result] if result else []
        if not lines:
            return [], "rapidocr returned no text"
        return lines, ""
...
def detect_tipo(lines: List[str]) -> str:
    ...
    if "tunki" in joined:
        return "tunki"
    if "lemonpay" in joined:
        return "lemonpay"
    if "bcp" in joined or "banco de credito" in joined:
        return "bcp"
    if "interbank" in joined:
        return "interbank"
    if "scotiabank" in joined or "scotia" in joined:
        return "scotiabank"
    if "bbva" in joined:
        return "bbva"
    return ""
...
def main():
    if len(sys.argv) < 2:
        ...
        sys.exit(0)
    img_path = sys.argv[1]
    lines, err_msg = ocr_with_rapidocr(img_path)
