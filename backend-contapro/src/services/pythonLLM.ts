...
export async function runPythonLLM(app: FastifyInstance, buffer: Buffer, mimeType: string): Promise<PyLlmResult> {
  try {
    ...
    try {
      const parsed = JSON.parse(out) as PyLlmResult & { observaciones?: string };
      // Anexar trazas del comando Python efectivo usado para diagnóstico
      const cmdInfo = `py_cmd:${pythonCmd}`;
      if (parsed) {
        parsed.observaciones = parsed?.observaciones ? `${parsed.observaciones} | ${cmdInfo}` : cmdInfo;
      }
