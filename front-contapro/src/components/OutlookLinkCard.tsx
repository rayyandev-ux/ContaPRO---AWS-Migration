...
export default function OutlookLinkCard({ maxSlots = 1 }: { maxSlots?: number }) {
  ...
  async function refresh() {
    setLoading(true);
    try {
      const res = await fetch("/api/proxy/integrations/outlook/status", { credentials: "include" });
  ...
  async function updateSettings(newSenders: string[], newAutoApprove?: boolean, newCustomName?: string) {
    ...
    try {
        const res = await fetch("/api/proxy/integrations/outlook/settings", { 
            method: "PUT", 
  ...
  async function handleDisconnect() {
    ...
    setLoading(true);
    try {
      await fetch("/api/proxy/integrations/outlook/disconnect", { 
        method: "POST", 
        headers: { "Content-Type": "application/json" },
  ...
  async function handleTest() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/proxy/integrations/outlook/test", { 
        method: "POST", 
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: selectedEmail }),
        credentials: "include" 
